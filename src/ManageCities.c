 ...
 * are used to remove a city from the graph or to rename it.
 */
...
void removeCity(Graph_t* graph, const char* city)
{
    ...
    for (upper = graph->head; upper != NULL; upper = upper->next) {
        ...
        if (strcmp(upper->from, city) == 0) {
            ...
            /* Now, 2 cases: if given city creates the first adjacency list in the graph... */
...
void renameCityMenu(Graph_t* graph)
{
    ...
    do {
        ...
        if (what_error == NO_ERROR) {
            what_error = checkIfNamesAreIdentical(city, new_citys_name);
        }
        if (what_error == NO_ERROR) {
