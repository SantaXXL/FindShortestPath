 ...
 * DESCRIPTION:
 *
 * This file contains definitions of addConnection function
 * and other functions that are needed to add a connection to the graph.
 */
 ...
 *
 * DESCRIPTION:
 * Function gets from user names of cities, that create a
...
void addOneCityToTheGraph(Graph_t* graph, const char* city1, const char* city2, double distance)
{
    ...
    if (true == is_city1_in_the_graph) {
        ...
        for (adjacency_list = graph->head; adjacency_list != NULL; adjacency_list = adjacency_list->next) {
            ...
            if (NULL != adjacency_list->next and strcmp(city2, adjacency_list->from) > 0 and strcmp(city2, adjacency_list->next->from) < 0) {
    ...
    /* Case, when city2 is already in the graph. */
        ...
        for (adjacency_list = graph->head; adjacency_list != NULL; adjacency_list = adjacency_list->next) {
            ...
            if (NULL != adjacency_list->next and strcmp(city1, adjacency_list->from) > 0 and strcmp(city1, adjacency_list->next->from) < 0) {
        ...
        if (strcmp(city1, city2) < 0) {
            ...
            for (adjacency_list = graph->head; adjacency_list != NULL; adjacency_list = adjacency_list->next) {
                ...
                if (NULL != adjacency_list->next and strcmp(city1, adjacency_list->from) > 0 and strcmp(city1, adjacency_list->next->from) < 0) {
                    ...
                    temp->next = adjacency_list->next;
                    adjacency_list->next = temp;
                } else if (NULL != adjacency_list->next and strcmp(city2, adjacency_list->from) > 0 and strcmp(city2, adjacency_list->next->from) < 0) {
                    AdjacencyList_t* temp = makeAdjacencyList(city2);
                    addNodeToAdjacencyList(temp, city1, distance);
                    temp->next = adjacency_list->next;
                    adjacency_list->next = temp;
        ...
        /* Case 2: city2 is before city1 (in alphabetical order)
            ...
            for (adjacency_list = graph->head; adjacency_list != NULL; adjacency_list = adjacency_list->next) {
                ...
                if (NULL != adjacency_list->next and strcmp(city2, adjacency_list->from) > 0 and strcmp(city2, adjacency_list->next->from) < 0) {
                    ...
                    temp->next = adjacency_list->next;
                    adjacency_list->next = temp;
                } else if (NULL != adjacency_list->next and strcmp(city1, adjacency_list->from) > 0 and strcmp(city1, adjacency_list->next->from) < 0) {
                    AdjacencyList_t* temp = makeAdjacencyList(city1);
                    addNodeToAdjacencyList(temp, city2, distance);
                    temp->next = adjacency_list->next;
                    adjacency_list->next = temp;
