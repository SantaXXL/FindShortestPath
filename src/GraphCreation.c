 ...
 * to create graph, add edges and so on.
 * Firstly, function graphCreation will be launched.
 * Then graph will be created and edges will be added.
 */
...
Graph_t *graphCreation(void)
{
     ...
     * Cities from unique_cities_list will create a nodes
    ...
    /* Free memory and return created graph. */
...
void updateDataBaseFile(const Graph_t *graph)
{
    ...
    {
        printErrorDataBase(CANT_UPDATE_FILE);
        return;
    }
 ...
 *
 * DESCRIPTION:
 * Function updates ordinal numbers of the cities in the graph.
 */
...
void updateOrdinalNumbers(Graph_t *graph)
{
    ...
    /* Firstly, set ordinal numbers in "upper" list. */
    ...
    for(upper = graph->head; upper != NULL; upper = upper->next)
    {
        ...
        for(lower = upper->head; lower != NULL; lower = lower->next)
        {
            ...
            {
                /* If names are the same (elements are matched) then update
